    ...
    return (
        <>
            ...
            <div className='conteiner_emprestimo'>
                <input type="text" placeholder='Email' onChange={(e) => user.email = e.target.value}/>
                <input type="text" placeholder='Senha' onChange={(e) => user.senha = e.target.value}/>
                <div>
                    {epis.map((epi) => (
                        <div className='epi_emprestimo'>
                            <img src={epi.foto} className='img_emprestimo' />
                            <div className='especificacoes_epis'>
                                <p>{epi.nome}</p>
                                <div className='conteiner_quantidade_emprestimo'>
                                    <div onClick={() => remover_pegos(epi.id)} className='botoes_empestimo'>-</div>
                                    <p>{epi.qtd}</p>
                                    <div onClick={() => add_pegos(epi.id)} className='botoes_empestimo'>+</div>
                                </div>
                                {pegos.map((i) => i.qtd > 0 ? i.id == epi.id ? (<p>Quantidade Pega: {i.qtd}</p>) : null : null)}
                            </div>
                        </div>
                    ))}
                </div>
